...
export const friendService = {
  getMyFriends: async (): Promise<Friend[]> =>
    api.get<Friend[]>('/api/v1/friends').then(r => r.data),
  ...
  getAcquaintances: async (userId: number): Promise<Acquaintance[]> =>
    api.get<Acquaintance[]>(`/api/v1/users/${userId}/acquaintances`).then(r => r.data),

  getUserProfile: async (userId: number): Promise<UserProfile | null> =>
    api.get<UserProfile>(`/api/v1/users/${userId}/profile`).then(r => r.data),

  sendContactRequest: async (targetUserId: number): Promise<ContactRequest> =>
    api.post<ContactRequest>('/api/v1/contact-requests', { targetUserId }).then(r => r.data),

  acceptContactRequest: async (requestId: number): Promise<AcceptContactResponse> =>
    api.post<AcceptContactResponse>(`/api/v1/contact-requests/${requestId}/accept`).then(r => r.data),

  rejectContactRequest: async (requestId: number): Promise<void> =>
    api.post(`/api/v1/contact-requests/${requestId}/reject`).then(() => undefined),

  getSentRequests: async (): Promise<ContactRequest[]> =>
    api.get<ContactRequest[]>('/api/v1/contact-requests/sent').then(r => r.data),

  getReceivedRequests: async (): Promise<ReceivedContactRequest[]> =>
    api.get<ReceivedContactRequest[]>('/api/v1/contact-requests/received').then(r => r.data),

  getMyProfile: async (): Promise<MyProfile> =>
    api.get<MyProfile>('/api/v1/users/me/profile').then(r => r.data),

  updateMyProfile: async (data: MyProfile): Promise<void> =>
    api.put('/api/v1/users/me/profile', data).then(() => undefined),

  checkNickname: async (nickname: string): Promise<{ available: boolean }> =>
    api.get<{ available: boolean }>(`/api/v1/users/nickname-check?nickname=${nickname}`).then(r => r.data),
};
