...
> 스택: Spring Boot 3.3 + Java 21 + Lombok + PostgreSQL + JWT + Flyway. 레이어드 아키텍처(Controller → Service → Repository → Domain).

## 패키지 구조 (`com.dash`)

| 패키지 | 역할 | 상태 |
|--------|------|------|
| `member` | 회원 엔티티/리포지토리 (kakaoId 기반) | 엔티티만 |
| `profile` | 내 프로필 (조회/수정/닉네임확인) | ✅ |
| `friendship` | 친구 관계·친구 목록 | ✅ |
| `user` | 타 유저 프로필·지인 탐색 (읽기 전용) | ✅ |
| `contactrequest` | 연락 요청 (보내기/수락/거절) | ✅ |
| `invitation` | 친구 초대 링크 | ✅ |
| `global` | 보안(JWT)·예외·Swagger 설정 | — |
...
- 마이그레이션: Flyway `src/main/resources/db/migration/` (V1 init, V2 profiles, V3 contact_requests).
- 엔티티: `Member`(members), `Profile`(profiles, 공유 PK member_id), `Friendship`(friendships, member_a_id<member_b_id), `ContactRequest`(contact_requests), `Invitation`(invitations).
